...
@fused.udf
def udf(
    lat: float = 49.2426,  # Default to Vancouver
    lon: float = -123.1119,
):
    """Calculate max, median, and average tree height within 250m buffer of given lat/lon using proper geospatial buffering"""
    import numpy as np
    ...
    max_height = round(float(arr.max()), 2)
    median_height = round(float(np.median(arr)), 2)
    mean_height = round(float(np.mean(arr)), 2)
    ...
    result = pd.DataFrame({
        ...
        'median_tree_height_m': [median_height],
        'mean_tree_height_m': [mean_height],
        'meta_chm_tile_id': [image_id['tile'].iloc[0]]
    })

    print(f"Max tree height within {buffer_meters}m buffer: {max_height}m")
    print(f"Median tree height within {buffer_meters}m buffer: {median_height}m")
    print(f"Mean tree height within {buffer_meters}m buffer: {mean_height}m")
    print(f"{result=}")
